...
##cross links::  [(u'include Oyr 3D tracers', u'Omon'), (u'include Amon 2D', u'cf3hr'), (u'include Amon 2D', u'cfSites')]
  ...
  def get_cfmip_request_spec(self):
    ...
    if self.rei[1] not in keys:
      ...
      print self.rei[1],' not in keys:: ',keys
      return False
...
## offset_status has 3 levels: -1: not set, 0: set by default, 1: set using info from configuration file.
    ...
    if verbose and table == 'cfMon':
      print 'find_product_step_one::', var, self.table_segment
    ...
    if verbose:
       print 'need to identify experiment'
  ...
  def find_product_slice(self):
    ...
    if self.verbose:
       print 'find_product_slice: years requested: %s, years submitted: %s' % (self.nyears_requested, self.nyears_submitted)
...
### take last 35 years
    ...
    if len(self.expt) > 7 and self.expt[0:7] == 'decadal':
      ...
      if self.verbose:
         print 'Experiment %s, time dataum %s' % (self.expt, self.time_datum )
